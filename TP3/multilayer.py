...
class Multilayer:
    def __init__(self, layer_sizes, pattern_size): # layer_sizes = [2, 1]
        self.layer_sizes = layer_sizes
        self.layers = []
        for idx, layer_size in enumerate(layer_sizes):
            is_first_layer = idx == 0
            self.layers.append(Layer(0.5, layer_size, (pattern_size if idx == 0 else layer_sizes[idx - 1]) + 1, is_first_layer, idx == len(layer_sizes) - 1))
